...
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.ContactDAO;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.FragmentExecuotr;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.PermissionsChecker;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.RingtonePlayer;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.UsersUtils;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.WebRtcSessionManager;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.callbacks.ConversationFragmentCallbackListener;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.callbacks.IncomeCallFragmentCallbackListener;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.callbacks.NetworkConnectionChecker;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.callbacks.OnCallEventsController;
...
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.fragment.AudioConversationFragment;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.fragment.BaseConversationFragment;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.fragment.IncomeCallFragment;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.fragment.ScreenShareFragment;
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.fragment.VideoConversationFragment;
...
public class CallActivity extends BaseActivity implements QBRTCClientSessionCallbacks, QBRTCSessionStateCallback, QBRTCSignalingCallback,
    ...
    boolean isCallPicked = false;
    String callRecieveTime = "";
...
//            we have already currentSession == null, so it's no reason to do further initialization
    ...
    private void startSuitableFragment(boolean isInComingCall) {
        if (isInComingCall) {
            ...
            checkPermission();
            callRecieveTime = System.currentTimeMillis() + "";
        } else {
...
//        PermissionsActivity.startActivity(this, checkOnlyAudio, Consts.PERMISSIONS);
    ...
    private void startLoadAbsentUsers() {
        SQLiteDatabase database = DatabaseHelper.getDatabaseInstanse(getApplicationContext()).getReadableDatabase();
//        ArrayList<QBUser> usersFromDb = dbManager.getAllUsers();
        ArrayList<QBUser> usersFromDb = ContactDAO.getInstance().getAllContact(database);
        ...
        if (!idsUsersNeedLoad.isEmpty()) {
            requestExecutor.loadUsersByIds(idsUsersNeedLoad, new QBEntityCallbackImpl<ArrayList<QBUser>>() {
                ...
                public void onSuccess(ArrayList<QBUser> result, Bundle params) {
                    SQLiteDatabase database = DatabaseHelper.getDatabaseInstanse(getApplicationContext()).getReadableDatabase();
                    ContactDAO.getInstance().saveAllUsers(CallActivity.this.getApplicationContext(), result);
...
//        dbManager = QbUsersDbManager.getInstance(getApplicationContext());
    ...
    private void initAudioManager() {
            ...
            public void onAudioChangedState(AppRTCAudioManager.AudioDevice audioDevice) {
                if (callStarted) {
                    ...
                    if (showToastAfterHeadsetPlugged) {
                        CommonUtils.getInstance(CallActivity.this).ShowToast("Audio device switched to  " + audioDevice);
                    }
                }
            }
        });
        ...
        audioManager.setOnWiredHeadsetStateListener(new AppRTCAudioManager.OnWiredHeadsetStateListener() {
            ...
            public void onWiredHeadsetStateChanged(boolean plugged, boolean hasMicrophone) {
                ...
                if (callStarted) {
                    CommonUtils.getInstance(CallActivity.this).ShowToast("Headset " + (plugged ? "plugged" : "unplugged"));
                }
    ...
    private void initQBRTCClient() {
        ...
        XmppUtils.setSettingsStrategy(opponentsIdsList, sharedPref, CallActivity.this);
        XmppUtils.configRTCTimers(CallActivity.this);
    ...
    private void setExpirationReconnectionTime() {
        reconnectHangUpTimeMillis = XmppUtils.getPreferenceInt(sharedPref, this, R.string.pref_disconnect_time_interval_key,
    ...
    private void initIncomingCallTask() {
        ...
        showIncomingCallWindowTask = new Runnable() {
            ...
            public void run() {
                ...
                CommonUtils.getInstance(CallActivity.this).ShowToast(("Call was stopped by timer"));
            }
        };
    }
    ...
    public void onSessionClosed(final QBRTCSession session) {
        ...
        if (session.equals(getCurrentSession())) {
            ...
            userIds.add(currentSession.getCallerID());


            SQLiteDatabase database = DatabaseHelper.getDatabaseInstanse(getApplicationContext()).getReadableDatabase();
            QBUser user = ContactDAO.getInstance().getUserById(database, currentSession.getCallerID());
            database = DatabaseHelper.getDatabaseInstanse(getApplicationContext()).getReadableDatabase();
            CallHistoryDAO dao = new CallHistoryDAO();
            dao.oppoName = user.getFullName();
            if (user.getFileId() != null) {
                dao.oppoImage = user.getFileId() + "";
            }
            dao.callTime = callRecieveTime;
            if (isCallPicked) {
                dao.callMode = Consts.ChatValues.CALL_RECEIVED;
            } else {
                dao.callMode = Consts.ChatValues.CALL_MISSED;
            }
            dao.oppoID = TextUtils.join(",", userIds);
            if (isVideoCall) {
    ...
    private void showToast(final int message) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                CommonUtils.getInstance(CallActivity.this).ShowToast(getString(message));
            }
        });
    }
    ...
    private void showToast(final String message) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                CommonUtils.getInstance(CallActivity.this).ShowToast(message);
            }
        });
    }
    ...
    public void onReceiveHangUpFromUser(final QBRTCSession session, final Integer userID, Map<String, String> map) {
        if (session.equals(getCurrentSession())) {
            ...
            SQLiteDatabase database = DatabaseHelper.getDatabaseInstanse(this).getReadableDatabase();
            QBUser participant = ContactDAO.getInstance().getUserById(database, userID);
