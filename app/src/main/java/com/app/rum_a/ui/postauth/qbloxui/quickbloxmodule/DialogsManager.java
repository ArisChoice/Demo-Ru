...
import com.app.rum_a.ui.postauth.qbloxui.quickbloxmodule.callbacks.QbEntityCallbackImpl;
    ...
    public void sendSystemMessageAboutCreatingDialog(QBSystemMessagesManager systemMessagesManager, QBChatDialog dialog) {
        ...
        try {
            for (Integer recipientId : dialog.getOccupants()) {
                if (!recipientId.equals(ChatHelper.getCurrentUser().getId())) {
...
//                QbDialogHolder.getInstance().updateDialog(dialogId, chatMessage);
                notifyListenersDialogUpdated(QbDialogHolder.getInstance().updateDialog(dialogId, chatMessage));
            } else {
    ...
    private void notifyListenersDialogUpdated(final QBChatDialog dialogId) {
        for (ManagingDialogsCallbacks listener : getManagingDialogsCallbackListeners()) {
    ...
    public interface ManagingDialogsCallbacks{
        ...
        void onDialogUpdated(QBChatDialog chatDialog);
